#!/usr/bin/env python3
"""
Generic ZIP Inference Script

Runs inference on any ZIP holdout dataset using the best configuration.
...
def main():
    parser = argparse.ArgumentParser(description="Run inference on ZIP holdout dataset")
    ...
    parser.add_argument("--output-prefix", default=None, help="Output file prefix (default: input filename)")
    args = parser.parse_args()
    
    if args.output_prefix is None:
        args.output_prefix = Path(args.input).stem
    
    output_csv = f"/workspace/{args.output_prefix}_predictions.csv"
    output_json = f"/workspace/{args.output_prefix}_predictions.json"
    
    inference = GenericZipInference(args.qdrant_path)
    inference.run_inference_on_zip(args.input)
    inference.save_results_csv(output_csv)
    inference.save_results_json(output_json)
